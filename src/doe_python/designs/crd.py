...
class CompletelyRandomizedDesign(ExperimentalDesign):
    ...
    def __init__(self, treatments: List[str], replicates: int):
        ...
        -----------
        treatments : List[str]
            List of treatment names/levels
        replicates : int
    ...
    def create_data_collection_sheet(self, response_variables: List[str] = None) -> pd.DataFrame:
        """
    ...
    def summary_statistics(self, data: pd.DataFrame, response_column: str) -> pd.DataFrame:
        """
        ...
        summary = data.groupby('Treatment')[response_column].agg([
