...
class ModelFitting:
    ...
    def __init__(self, data: pd.DataFrame, response_column: str):
        ...
        self.factor_columns = [col for col in data.columns 
                              if col not in [response_column, 'RunID', 'RunOrder', 'Replicate', 'DesignPoint']]
    ...
    def stepwise_selection(self, entry_threshold: float = 0.05, 
                          removal_threshold: float = 0.10,
                          max_terms: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        entry_threshold : float, default=0.05
            P-value threshold for entering terms
        removal_threshold : float, default=0.10
            P-value threshold for removing terms
    ...
    def hierarchical_fitting(self, max_order: int = 3, 
        ...
        significance_level : float, default=0.05
            Significance level for term inclusion
                        ...
                        if term in model_result['p_values']:
                            ...
                            if p_value < significance_level:
                                significant_terms.append({
                                    'term': term,
                                    'p_value': p_value,
                                    'coefficient': model_result['coefficients'][term]
                                })
                    except:
                        continue
    ...
    def all_subsets_selection(self, criterion: str = 'AIC') -> Dict[str, Any]:
        ...
        if len(candidate_terms) > 20:
            warnings.warn("Large number of candidate terms. Consider using stepwise selection.")
        ...
        overall_best = None
        overall_best_criterion = float('inf') if criterion in ['AIC', 'BIC'] else float('-inf')
    ...
    def cross_validation(self, model_terms: List[str], k_folds: int = 5, 
        ...
        for fold, (train_idx, test_idx) in enumerate(kf.split(self.data)):
            ...
            try:
                ...
                test_predictions = self._predict_with_model(train_model, test_data, model_terms)
                ...
                fold_r2 = 1 - np.sum((test_actuals - test_predictions)**2) / np.sum((test_actuals - np.mean(test_actuals))**2)
        ...
        if cv_results['predictions']:
            ...
            cv_results['overall_rmse'] = np.sqrt(np.mean((all_predictions - all_actuals)**2))
            cv_results['overall_mae'] = np.mean(np.abs(all_predictions - all_actuals))
            cv_results['overall_r2'] = 1 - np.sum((all_actuals - all_predictions)**2) / np.sum((all_actuals - np.mean(all_actuals))**2)
    ...
    def bootstrap_validation(self, model_terms: List[str], n_bootstrap: int = 100,
                           random_state: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        for bootstrap_idx in range(n_bootstrap):
            ...
            try:
                ...
                # Store model metrics
                bootstrap_results['r_squared'].append(bootstrap_model['model_metrics']['R2'])
                bootstrap_results['rmse'].append(bootstrap_model['model_metrics']['RMSE'])
                
                # Predict on original data
                predictions = self._predict_with_model(bootstrap_model, self.data, model_terms)
        ...
        for term in model_terms:
            coeff_values = [c for c in bootstrap_results['coefficients'][term] if not np.isnan(c)]
            if coeff_values:
        ...
        for i, model_terms in enumerate(model_list):
            try:
                ...
                comparison_results.append({
                    ...
                    'F_Statistic': model_result['model_metrics'].get('F_statistic', np.nan),
                    'F_P_Value': model_result['model_metrics'].get('F_p_value', np.nan)
                })
    ...
    def _predict_with_model(self, model_result: Dict[str, Any], 
                           new_data: pd.DataFrame, model_terms: List[str]) -> np.ndarray:
    ...
    def residual_diagnostics(self, model_terms: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            diagnostics['normality_test'] = {'error': 'Unable to perform normality test'}
        
        # Homoscedasticity tests
        try:
            # Breusch-Pagan test
            from statsmodels.stats.diagnostic import het_breuschpagan
            bp_stat, bp_p, bp_f_stat, bp_f_p = het_breuschpagan(residuals, model_result['model_object'].model.exog)
            ...
            diagnostics['homoscedasticity_test'] = {'error': 'Unable to perform homoscedasticity test'}
        ...
        try:
            ...
            diagnostics['independence_test'] = {'error': 'Unable to perform independence test'}
        ...
        try:
            ...
            diagnostics['leverage_influence'] = {'error': 'Unable to calculate leverage and influence'}
