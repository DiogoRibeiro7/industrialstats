...
from doe_python.designs.response_surface import ResponseSurfaceDesign
from doe_python.designs.base import Factor
from doe_python.analysis.model_fitting import ModelFitting
from doe_python.visualization.plots import ExperimentPlotter
...
def main():
    ...
    print(f"Experimental data collected!")
    ...
    print("-" * 30)
    results_display = design_matrix[['Temperature', 'Pressure', 'Yield', 'PointType']].round(1)
    ...
    if optimum_actual:
        ...
        pressure_within = pressure_range[0] <= optimum_actual['Pressure'] <= pressure_range[1]
    ...
    print(f"Factor Importance (relative contribution to response):")
    ...
    if optimum_actual:
        ...
        print(f"Response sensitivity to ±5% changes in factors:")
        ...
        print(f"• Implement process monitoring for all critical factors")
        print(f"• Consider operating slightly away from optimum for robustness")
    ...
    try:
        create_rsm_visualizations(design_matrix, rsm_design, coefficients, 
                                optimum_actual, factors)
    ...
    if optimum_actual:
        print(f"1. CONFIRMATION EXPERIMENTS:")
        print(f"   • Run 3-5 replicates at optimal conditions")
        print(f"   • Expected yield: {optimal_yield:.1f} ± 2.0%")
        
        print(f"\n2. ROBUSTNESS VALIDATION:")
        print(f"   • Test small deviations from optimal point")
        print(f"   • Verify process stability over time")
        
        print(f"\n3. SCALE-UP CONSIDERATIONS:")
        print(f"   • Validate model at pilot scale")
        print(f"   • Monitor for scale-dependent effects")
        print(f"   • Update model with production data")
    
    print(f"\n4. PROCESS CONTROL:")
    print(f"   • Implement SPC charts for critical factors")
    print(f"   • Establish control limits based on sensitivity analysis")
    print(f"   • Regular model validation with new data")
    ...
    print(f"2. Significant Main Effects: {', '.join(main_effects) if main_effects else 'None'}")
    print(f"3. Significant Interactions: {', '.join(interactions) if interactions else 'None'}")
    
    # Optimization result
    if optimum_actual:
        print(f"4. Optimization: Successful - {optimal_yield:.1f}% yield predicted")
    else:
        print(f"4. Optimization: Stationary point not found in region")
    
    print(f"\n💡 STRATEGIC RECOMMENDATIONS:")
    print("-" * 35)
    
    if optimum_actual and improvement > 2:
        print(f"• HIGH PRIORITY: Implement optimal conditions immediately")
        print(f"• Expected yield improvement: +{improvement:.1f} percentage points")
    elif optimum_actual:
        print(f"• MODERATE PRIORITY: Consider optimization implementation")
        print(f"• Modest improvement expected: +{improvement:.1f} percentage points")
    else:
        print(f"• EXPLORATION NEEDED: Expand experimental region")
        print(f"• Current region may not contain optimum")
    
    print(f"• Establish robust process control for critical factors")
    print(f"• Validate model predictions with confirmation experiments")
    print(f"• Consider advanced optimization techniques if needed")
...
def predict_yield_at_conditions(conditions: dict, coefficients: dict) -> float:
    ...
    # Interaction term
    prediction += coefficients.get('Temperature*Pressure', 0) * temp_coded * pressure_coded
...
def analyze_robustness(optimal_conditions: dict, coefficients: dict, factors: list) -> dict:
...
def create_rsm_visualizations(design_matrix: pd.DataFrame, rsm_design, 
                            coefficients: dict, optimum_actual: dict, factors: list):
    ...
    for point_type in point_types:
        ...
        ax1.scatter(subset['Temperature'], subset['Pressure'], 
                   c=colors[point_type], marker=markers[point_type],
                   s=100, alpha=0.7, label=point_type, edgecolors='black')
    
    # Add optimum if found
    if optimum_actual:
        ax1.scatter(optimum_actual['Temperature'], optimum_actual['Pressure'],
                   c='gold', marker='*', s=300, edgecolors='black', linewidth=2,
    ...
    # Create contour data
    X, Y, Z = rsm_design.contour_data(coefficients, 'Temperature', 'Pressure', grid_size=25)
    
    # 3D surface
    surface = ax2.plot_surface(X, Y, Z, cmap='viridis', alpha=0.8, linewidth=0)
    
    # Add experimental points
    ax2.scatter(design_matrix['Temperature'], design_matrix['Pressure'], 
               design_matrix['Yield'], c='red', s=50, alpha=0.8)
    
    # Add optimum
    if optimum_actual:
        optimal_yield = predict_yield_at_conditions(optimum_actual, coefficients)
        ax2.scatter(optimum_actual['Temperature'], optimum_actual['Pressure'],
                   optimal_yield, c='gold', marker='*', s=200)
    ...
    # Add experimental points
    scatter = ax3.scatter(design_matrix['Temperature'], design_matrix['Pressure'], 
                         c=design_matrix['Yield'], s=100, cmap='viridis', 
                         edgecolors='white', linewidth=1, zorder=5)
    
    # Add optimum
    if optimum_actual:
        ax3.scatter(optimum_actual['Temperature'], optimum_actual['Pressure'],
                   c='gold', marker='*', s=300, edgecolors='black', linewidth=2,
                   zorder=6)
    ...
    try:
        ...
        predicted_yields = []
        for _, row in design_matrix.iterrows():
            conditions = {'Temperature': row['Temperature'], 'Pressure': row['Pressure']}
    ...
    try:
        ...
        variance_grid = np.sqrt((T_grid - center_temp)**2 / 2500 + (P_grid - center_pressure)**2 / 9)
        
        contour_var = ax5.contour(T_grid, P_grid, variance_grid, levels=10, colors='black', alpha=0.4)
        ax5.clabel(contour_var, inline=True, fontsize=8)
        contourf_var = ax5.contourf(T_grid, P_grid, variance_grid, levels=10, cmap='Reds', alpha=0.7)
        
        # Add design points
        ax5.scatter(design_matrix['Temperature'], design_matrix['Pressure'], 
                   c='blue', s=80, alpha=0.8, edgecolors='white', linewidth=1)
