...
from doe_python.analysis.effects import EffectsAnalysis
...
def main():
    ...
    for factor in factors:
        levels_str = f"{factor.levels[0]} - {factor.levels[1]}" if factor.factor_type == "continuous" else ", ".join(map(str, factor.levels))
    ...
    sorted_interactions = sorted(two_factor.items(), key=lambda x: abs(x[1]), reverse=True)
    ...
    model_formula = ('Tensile_Strength ~ C(Temperature) * C(Pressure) * ' +
    ...
    if improvement > 0:
        ...
        print(f"Note: Predicted optimum is within current experimental range")
    ...
    sorted_all_effects = sorted(all_effects.items(), key=lambda x: abs(x[1]), reverse=True)
    ...
    critical_factors = [effect for effect, _ in sorted_all_effects[:3] if '*' not in effect]
    print(f"Critical factors requiring tight control:")
    for factor in critical_factors:
        print(f"  • {factor}")
    
    # Robust settings
    print(f"\nFor robust operation (less sensitive to variation):")
    robust_recommendations = get_robust_recommendations(main_effects, interaction_effects)
    ...
    try:
        create_comprehensive_plots(design_matrix, effects_analyzer, main_effects, 
                                 interaction_effects, model)
    ...
    print(f"\n🔑 KEY FINDINGS:")
    print("-" * 20)
    print(f"1. Most important factor: {most_important[0]} ({most_important[1]:+.1f} MPa effect)")
    
    if most_important_interaction:
        print(f"2. Strongest interaction: {most_important_interaction[0]} " +
              f"({most_important_interaction[1]:+.1f} MPa effect)")
    
    print(f"3. Optimal settings identified for {predicted_response:.1f} MPa tensile strength")
    ...
    if 'Temperature*Material' in interaction_effects:
        ...
        if abs(temp_mat_interaction) > 1:
            print(f"• Temperature effect depends on material choice")
...
def simulate_tensile_strength_data(design_matrix: pd.DataFrame) -> np.ndarray:
    ...
    for _, row in design_matrix.iterrows():
        ...
        temp_material = 4.0 if (row['Temperature'] > 200 and row['Material'] == 'ABS') else 0.0
        pressure_cooling = ((row['Pressure'] - 100) * (row['Cooling_Time'] - 15)) * -0.002
        
        # Calculate total response
        total_response = (base_strength + temp_effect + pressure_effect + 
                         cooling_effect + material_effect + temp_pressure + 
                         temp_material + pressure_cooling)
    ...
    for factor in factors:
        if factor.name in main_effects:
            ...
            if factor.factor_type == 'categorical':
                ...
                optimal[factor.name] = factor.levels[1] if effect > 0 else factor.levels[0]
        else:
            # Default to high level if no effect calculated
            optimal[factor.name] = factor.levels[1] if factor.factor_type == 'continuous' else factor.levels[0]
                
    return optimal


def predict_optimal_response(optimal_conditions: dict, main_effects: dict, 
                           interaction_effects: dict) -> float:
    """Predict response at optimal conditions."""
...
def get_robust_recommendations(main_effects: dict, interaction_effects: dict) -> list:
    ...
    strong_interactions = [name for name, effect in interaction_effects.items() 
...
def create_comprehensive_plots(design_matrix: pd.DataFrame, effects_analyzer, 
                             main_effects: dict, interaction_effects: dict, 
                             model=None):
    ...
    try:
        ...
        plt.subplot(2, 2, 1)
        create_interaction_subplot(design_matrix, 'Temperature', 'Material', 'Tensile_Strength')
        
        # Temperature vs Pressure interaction  
        plt.subplot(2, 2, 2)
        create_interaction_subplot(design_matrix, 'Temperature', 'Pressure', 'Tensile_Strength')
        
        # Pressure vs Cooling Time interaction
        plt.subplot(2, 2, 3)  
        create_interaction_subplot(design_matrix, 'Pressure', 'Cooling_Time', 'Tensile_Strength')
        
        # Material vs Pressure interaction
        plt.subplot(2, 2, 4)
        create_interaction_subplot(design_matrix, 'Material', 'Pressure', 'Tensile_Strength')
        
        plt.tight_layout()
        fig2.savefig('manufacturing_interaction_plots.png', dpi=300, bbox_inches='tight')
        plt.close(fig2)
    except Exception as e:
    ...
    try:
        all_effects = {**main_effects, **{k: v for k, v in interaction_effects.items() if k.count('*') == 1}}
    ...
    # 5. Design space plot
    try:
        fig5 = plotter.design_space_plot('Temperature', 'Pressure', 'Tensile_Strength', 
                                        figsize=(10, 8))
...
def create_interaction_subplot(data: pd.DataFrame, factor1: str, factor2: str, response: str):
    ...
    for level1 in levels1:
        ...
        if valid_indices:
            ...
            plt.plot(x_vals, y_vals, 'o-', label=f'{factor1}={level1}', linewidth=2, markersize=6)
